...
class WebsiteCalendar(models.Model):
    ...
    time_frame_ids = fields.One2many(
        ...
        required=True,
        string="Time Frames",
        help="Time frames that bookings are allowed for this user, can be multiple per day and different on each day",
    )

    def _compute_url(self):
        self.url = (
            request.httprequest.host_url + "book/calendar/" + str(self.id)
        )
