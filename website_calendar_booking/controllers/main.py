...
class WebsiteBookingController(http.Controller):
    @http.route(
        "/book/calendar/create", type="http", auth="public", website=True
    )
    def book_calendar_create(self, **kw):
        ...
        calendar = (
            request.env["website.calendar"]
            .sudo()
            .browse(int(values["calendar_id"]))
        )

        # worlds worse way of converting to UTC...
        parse_string = values["start"]
        if "+" in parse_string:
            start_date = parse(parse_string.replace("+", "-")).astimezone(
                pytz.utc
            )
    ...
    @http.route(
        "/book/calendar/<calendar>", type="http", auth="public", website=True
    )
    ...
    @http.route(
        "/book/calendar/timeframe/<calendar>",
        type="http",
        auth="public",
        website=True,
    )
    def book_calendar_timeframe(self, calendar, **kw):
        ...
        for time_frame in my_calendar.time_frame_ids:
            ...
            return_string += (
                '"start": "'
                + str(time_frame.start_time).replace(".", ":")
                + '",'
            )
            return_string += (
                '"end": "' + str(time_frame.end_time).replace(".", ":") + '",'
            )
